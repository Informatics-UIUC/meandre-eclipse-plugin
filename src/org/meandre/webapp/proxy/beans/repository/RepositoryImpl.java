...
public class RepositoryImpl implements QueryableRepository {
	...
	public RepositoryImpl ( Model mod ) {
		...
		//MeandreLogger.logInfo("Initializing the repository");
	...
	public void refreshCache () {
		// Flushing the cache
		//MeandreLogger.logInfo("Flushing the cached descriptions");
		htComDescMap.clear();
		setComRes.clear();
		htFlowDescMap.clear();
		setFlowRes.clear();
		...
		//MeandreLogger.logInfo("Refreshing cached descriptions");
		...
		for ( Resource res:setComRes ) {
			try {
				...
				//MeandreLogger.logError("Corrupted component found "+res.toString()+"\n"+e.toString());
			}
		}
		// Query all the flows
		...
		for ( Resource res:setFlowRes ) {
			try {
				...
				//MeandreLogger.logWarning("Corrupted flow found "+res.toString()+"\n"+e.toString());
			}
		}
		...
		*/
		...
		if ( setInputs.size()>0 ) {
			...
			*/
			...
			//MeandreLogger.logInfo("Component "+res.toString()+" has not firing policy, assuming all" );
			if ( lstLit.size()!=0 ) 
